-- Script pour créer les tables de paiement manquantes
...
CREATE POLICY IF NOT EXISTS "Users can view their own store revenue stats"
...
CREATE POLICY IF NOT EXISTS "Users can update their own store revenue stats"
ON public.store_revenue_stats
FOR UPDATE
USING (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can view their own store transactions"
...
CREATE POLICY IF NOT EXISTS "Users can insert their own store transactions"
ON public.store_transactions
FOR INSERT
WITH CHECK (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can view their own store withdrawals"
...
CREATE POLICY IF NOT EXISTS "Users can insert their own store withdrawals"
ON public.store_withdrawals
FOR INSERT
WITH CHECK (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can update their own store withdrawals"
ON public.store_withdrawals
FOR UPDATE
USING (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can view their own store bank accounts"
...
CREATE POLICY IF NOT EXISTS "Users can insert their own store bank accounts"
ON public.store_bank_accounts
FOR INSERT
WITH CHECK (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can update their own store bank accounts"
ON public.store_bank_accounts
FOR UPDATE
USING (
  store_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can delete their own store bank accounts"
ON public.store_bank_accounts
FOR DELETE
USING (
  store_id IN (
