-- 🚀 CRÉATION DE LA FONCTION check_storefront_last_modified
-- Date: 2025-01-28
-- Objectif: Fonction pour vérifier la dernière modification d'un storefront

-- Supprimer la fonction si elle existe déjà
DROP FUNCTION IF EXISTS check_storefront_last_modified(TEXT);

-- Créer la fonction de vérification des modifications
...
DECLARE
    ...
    categories_updated TIMESTAMP := NULL;
BEGIN
    ...
    -- Vérifier la dernière modification des catégories (si la table existe)
    BEGIN
        SELECT MAX(created_at) INTO categories_updated
        FROM categories 
        WHERE store_id = store_id_val;
    EXCEPTION 
        WHEN undefined_table THEN
            categories_updated := NULL;
    END;
    ...
    last_modified := GREATEST(
        ...
        COALESCE(categories_updated, '1900-01-01'::timestamp)
    );
...
'Retourne la dernière date de modification d''un storefront basée sur les modifications du store, templates, produits et catégories';
...
SELECT '✅ Fonction check_storefront_last_modified créée avec succès' as status;
